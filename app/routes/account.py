...
def register_account(
    ...
    _=Depends(check_auth([Role.SUPER_ADMIN])),
):
    """
...
async def bootstrap_core_api_with_an_account_with_all_roles(
    db: AsyncSession = Depends(get_db),
):
    if (
        await db.execute(select(Account).where(Account.username == "admin"))
    ).scalar_one_or_none():
        return
    password_hash = hash_password("admin")
    new_account = Account(
        username="admin", password=password_hash, display_name="Admin"
    )
