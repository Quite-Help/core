...
def _generate_alias() -> str:
    return ["Abebe", "Almaze", "Kebede"].pop()


@router.post("", response_model=AliasResponse)
async def create_or_get_alias(
    payload: AliasRequest, db: AsyncSession =  Depends(get_db), _: dict = Depends(check_auth([Role.SERVICE]))
):
    exisiting_alias = await alias_repo.get_by_telegram_user_id(db, payload.telegram_user_id)
    if exisiting_alias is not None:
        return AliasResponse(alias=exisiting_alias)
    new_alias = _generate_alias()
