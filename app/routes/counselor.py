...
from app.models.counselor import CounselorResponse, CounselorInfo, CreateCounselorRequest
...
async def get_counselors(db: AsyncSession = Depends(get_db), _: dict = Depends(check_auth([Role.ADMIN, Role.SERVICE]))):
...
async def get_counselor(
    counselorId: int, db: AsyncSession = Depends(get_db), _: dict = Depends(check_auth([Role.ADMIN, Role.SERVICE]))
):
...
@router.post("")
def create_counselor(body: CreateCounselorRequest, db = Depends(get_db), _ = Depends(check_auth([Role.ADMIN]))):
